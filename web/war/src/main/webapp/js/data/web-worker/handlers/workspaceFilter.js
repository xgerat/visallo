define(['require', 'underscore'], function(require, _) {
    ...
    return function(message) {
        require([
            ...
            var filterNumber = ++currentFilter,
                workspace = store.getObject(publicData.currentWorkspaceId, 'workspace'),
            ...
            ontology.ontology().done(function(ontology) {
                var result = partitionVertices(ontology, vertices, query, conceptFilter, propertyFilters),
                    ...
                    matchingVertexIds = _.pluck(matchingVertices, 'id'),
                    otherVertices = result.shift(),
                    otherVertexIds = _.pluck(otherVertices, 'id');

                _.defer(function() {
                    if (filterNumber !== currentFilter) {
                        return;
                    }

                    store.setFilteredVertexIds(workspace.workspaceId, otherVertexIds);

                    var vertices = store.getObjects(workspace.workspaceId, 'vertex', matchingVertexIds);
                    dispatchMain('rebroadcastEvent', {
                        eventName: 'selectObjects',
                        data: {
                            vertices: vertices,
                            options: {
                                fromFilter: true
    ...
    function isKindOfConcept(conceptsById, vertex, conceptTypeFilter) {
        var conceptTypeProperty = _.findWhere(vertex.properties, { name: 'http://visallo.org#conceptType' }),
