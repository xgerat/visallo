// Forked from
// https://github.com/cytoscape/cytoscape.js/blob/master/src/extensions/layout/concentric.js
//
// Fixes cases where straight lines can appear because of rings that have 1 or 2 nodes
define([], function() {

    let defaults = {
        fit: true, // whether to fit the viewport to the graph
        padding: 30, // the padding on fit
        startAngle: Math.PI / -2, // where nodes start in radians
    ...
    return function circleConcentricLayout(opts = {}) {
        const options = { ...defaults, ...opts };
        const { cy, eles } = options;
        const { center } = this.calculations({ nodeDimensionsIncludeLabels: true });
        ...
        for ( let i = 0; i < levels.length; i++ ){
            ...
            // Change the start position if the count in consecutuve levels is
            // 2 or less. Avoids multiple rings lining in straight line
            let startAngle = options.startAngle;
            if (i > 0 && level.length <= 2 && levels[i - 1].length <= 2) {
            ...
            for ( let j = 0; j < level.length; j++ ){
                let val = level[ j ];
                let theta = startAngle + (clockwise ? 1 : -1) * dTheta * j;
